...
`hybrid-harness`는 로컬 Qwen 계열 모델과 frontier 모델을 함께 쓰기 위한 harness 도구 모음입니다. 이 저장소는 Pi용 패키지와 OpenCode용 플러그인을 하나의 monorepo에서 관리합니다.
...
현재 두 패키지가 맞춰야 하는 핵심 개념은 다음과 같습니다.
